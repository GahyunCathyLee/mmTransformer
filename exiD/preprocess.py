...
def _first_numeric_series(df: pd.DataFrame, col: str, default, dtype):
...
def process_recording(rec_id: str, raw_dir: Path, temp_dir: Path, args):
    ...
    # ── Load CSVs ─────────────────────────────────────────────────────────────
    tracks = pd.read_csv(tracks_file, low_memory=False)
    ...
    raw_fps   = float(rmeta.loc[0, "frameRate"]) if "frameRate" in rmeta.columns else 25.0
    ...
    lane_centers_by_id = {}
    for lid in sorted(set(int(l) for l in lane_id if int(l) >= 0)):
        mask = lane_id == lid
        if np.any(mask):
            lane_centers_by_id[lid] = float(np.nanmean(y[mask]))
    ...
    nb_ids_all = np.stack([_first_numeric_series(tracks, c, -1, np.int32)[keep]
    ...
    all_lane_y = sorted(set(float(v) for v in lane_centers_by_id.values()))
    lane_segments = []
    lane_id2idx   = {}
...
def main():
    ...
    with ProcessPoolExecutor(max_workers=os.cpu_count()) as executor:
        results = list(tqdm(
            ...
            total=len(rec_ids), desc="Processing exiD",
        ))
        for res in results:
